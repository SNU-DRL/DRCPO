...
from . import augmentor_utils, database_sampler,  iter_occ_database_sampler


class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, reduced_info=None, object_augmentor=None, logger=None):
        self.root_path = root_path
        self.class_names = class_names
        self.logger = logger
        self.object_augmentor = object_augmentor
        self.reduced_info = reduced_info
    ...
    def gt_sampling(self, config=None):
        if config is not None and self.reduced_info is not None:
            config.update(dict(reduced_info=self.reduced_info))
    ...
    def iter_occ_sampling(self, config=None):
        if config is not None and self.reduced_info is not None:
            config.update(dict(reduced_info=self.reduced_info))
        db_sampler = iter_occ_database_sampler.IterOcclusionDataBaseSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
    ...
    def random_objects_augmentation(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_objects_augmentation, config=config)
        removed_objects_points = data_dict['points']
        local_patches = data_dict['local_patches']
        augmented_objects =[]
        augmentend_boxes = []
        for i, local_object in enumerate(local_patches):

            object_dict = dict(points=local_object, mask=False, bbox=data_dict['gt_boxes'][i])
            transform_sel = np.random.randint(0, len(self.object_augmentor))
            object_dict = self.object_augmentor[transform_sel](object_dict)
            augmented_objects.append(object_dict['points'])
            augmentend_boxes.append(object_dict['gt_boxes'].reshape(1,-1))

        augmented_objects.append(torch.from_numpy(removed_objects_points).float())
        data_dict['points'] = np.concatenate(augmented_objects, axis=0)
        data_dict['pseudo_boxes'] = np.concatenate(augmentend_boxes, axis=0)
    ...
    def forward(self, data_dict):
        ...
        #draw_scenes(data_dict['points'], data_dict['gt_boxes'])
