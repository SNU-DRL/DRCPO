    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]-1):
            ...
            if i == 0:
                ...
                ret_dict['%s_3d/overall_R40' % class_to_name[curcls]] = np.mean(
                    [mAP3d_R40[j, 0, 0], mAP3d_R40[j, 1, 0], mAP3d_R40[j, 2, 0]])
                ...
                ret_dict['%s_bev/overall_R40' % class_to_name[curcls]] = np.mean(
                    [mAPbev_R40[j, 0, 0], mAPbev_R40[j, 1, 0], mAPbev_R40[j, 2, 0]])
    """
    for j, curcls in enumerate(current_classes):
        # mAP threshold array: [num_minoverlap, metric, class]
        # mAP result: [num_class, num_diff, num_minoverlap]
        for i in range(min_overlaps.shape[0]):
            ...
            mAPbevmean = np.mean([mAPbev_R40[j, 0, i], mAPbev_R40[j, 1, i], mAPbev_R40[j, 2, i]])
            result += print_str((f"MEAN   :{mAPbevmean:.4f}, "))

            result += print_str((f"3d   AP:{mAP3d_R40[j, 0, i]:.4f}, "
                                 f"{mAP3d_R40[j, 1, i]:.4f}, "
                                 f"{mAP3d_R40[j, 2, i]:.4f}"))

            mAP3d_mean = np.mean([mAP3d_R40[j, 0, i], mAP3d_R40[j, 1, i], mAP3d_R40[j, 2, i]])
            result += print_str((f"MEAN   :{mAP3d_mean:.4f}, "))
            if compute_aos:
